...
- Agregar proyectos (uno o varios a la vez, llenado en tabla — una fila por proyecto, como en tu Excel).
- Editar y eliminar proyectos.
- Botón "Editar" con Datos del proyecto (misma tabla de una fila, más el Plan de acción) y Comentarios — ver sección "Nuevo formulario de proyecto (llenado en tabla)" más abajo.
...
## Nuevo formulario de proyecto (llenado en tabla)

Por pedido tuyo, el formulario de "Agregar/Editar proyecto" ya no usa secciones apiladas — ahora el llenado es **como una tabla / hoja de cálculo**, igual que en el Excel que me compartiste: cada proyecto es una fila, y cada dato (País, Categoría, Prioridad, Código, Nombre, Descripción, Ticket, Responsable, Área, Estado, Avance, Fecha de solicitud, Fecha de inicio, Vencimiento, Archivo/documento) es una columna con su campo editable directamente en la celda. La tabla tiene scroll horizontal (hay bastantes columnas) para que no se rompa el diseño en pantallas angostas. Al editar un proyecto hay dos pestañas:

- **Datos del proyecto** — la tabla de una sola fila con todos los datos del proyecto, y al final la sección **Plan de trabajo (opcional)** (al agregar un proyecto nuevo) o **Plan de acción** (al editar uno ya existente) — ver el detalle abajo.
- **Comentarios** — la bitácora de comentarios de siempre.

Al **agregar** proyectos, el botón "Agregar otro proyecto" ya no abre una pestaña nueva — agrega una fila más a la misma tabla, así que puedes capturar varios proyectos uno debajo del otro, como en una hoja de cálculo. Cada fila tiene, al final, un botón **"N tarea(s)"** que despliega debajo de la tabla el Plan de trabajo (opcional) de ese proyecto en particular — ahí puedes capturar de una vez las tareas del plan (nombre, responsable, fechas, dependencia, % de avance, estatus), que se guardan junto con su proyecto en cuanto haces clic en "Guardar". También puedes dejarlo vacío al crear el proyecto y agregar las tareas después.

Y al **editar** un proyecto ya existente, debajo de esa fila de datos aparece la sección **"Plan de acción"** con el Cronograma completo de ese proyecto — la misma tabla de tareas y botones de Excel/PDF que se ven al seleccionar la fila en la pantalla de Proyectos (ver la siguiente sección), pero ahora también disponible **desde dentro del formulario de Editar**, sin tener que cerrarlo. Agregar, editar o eliminar una tarea ahí se guarda de inmediato en la base de datos — no hace falta hacer clic en "Guardar cambios" para las tareas (ese botón es solo para los datos generales del proyecto).
